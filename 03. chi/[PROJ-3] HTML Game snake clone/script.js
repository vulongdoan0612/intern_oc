...
let snake,
  ...
  tails = [],
  score = 00,
  maxScore = window.sessionStorage.getItem("maxScore") || undefined,
  tickets = window.localStorage.getItem("tickets", 3),
  particles = [],
...
const initialize = () => {
  ...
  // dom_reset.addEventListener("click", reset, false);
  loop();
};
...
const gameOver = () => {
  ...
  window.sessionStorage.setItem("maxScore", maxScore);
...
dom_btnNo.addEventListener("click", () => {
  dom_modal.style.display = "none";
  dom_start.classList.add("btnActive");
});

dom_btnYes.addEventListener("click", () => {
  if (tickets > 0) {
    decreaseTickets();
    initialize();
    dom_modal.style.display = "none";
